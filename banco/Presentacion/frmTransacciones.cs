...
namespace banco.Presentacion
{
    public partial class frmTransacciones : Form
    {
        ...
        {
            DataTable tabla = oBD.ConsultarDb("SELECT * FROM tiposMovimientos order by 2");
        ...
        {
            DataTable tabla = oBD.ConsultarDb("SELECT * FROM cuentas where activo=1 order by 2");
        ...
        {
            int next = oBD.ProximaTransaccion();
            if (next > 0)
        ...
        private void GuardarTransaccion()
        {
            ...
            if (oBD.ConfirmarTransaccion(nuevo))
            {
                DataTable tabla = oBD.ConsultarDb("select total from cuentas where id_cuenta=" + nuevo.IdCuenta);
                ...
                if(oBD.EjecutarSP("sp_adjuntarTransaccion", lParametros) > 0 )
                {
