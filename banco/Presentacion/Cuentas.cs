...
namespace banco
{
    public partial class frmCuentas : Form
    {
        ...
        private void CargarGridCuentas()
        {
            ...
            lCuentas.Clear();
            DataTable tabla = oBD.ConsultarDb("Select cbu, t.nombre as 'Tipo de Cuenta', c.nombre + ' ' + c.apellido as 'cliente', total Total" +
                ...
                " where activo=1");
            DataTable tabla2 = oBD.ConsultarDb("select cbu, id_tipoCuenta, dni from cuentas c join clientes cc on c.id_cliente=cc.id_cliente where activo=1");
        ...
        {
            DataTable tabla = oBD.ConsultarDb("SELECT * FROM TiposCuentas order by 2");
        ...
        {
            DataTable tabla = oBD.ConsultarDb("SELECT * FROM clientes order by 2");
        ...
        private void btnGrabarCuentas_Click_1(object sender, EventArgs e)
        {
            if (ValidarCuenta())
            {
                ...
                if (!ExisteCuenta(c))
                {
                    ...
                    if (oBD.EjecutarSP("sp_ingresarCuenta", lParametros) > 0)
                    {
                ...
                else
                {
                    ...
                    lParametros.Add(new Parametros("@id_tipoCuenta", c.pTipoCuenta));
                    lParametros.Add(new Parametros("@id_cliente", c.pCliente));
                    if (oBD.EjecutarSP("sp_actualizarCuenta", lParametros) > 0) MessageBox.Show("Se ha podido actualizar la cuenta");
        ...
        private void btnEliminarCuentas_Click_1(object sender, EventArgs e)
        {
                ...
                == DialogResult.Yes)
            {
                ...
                if (oBD.EjecutarEliminarSP("Sp_eliminarCuentaYRelacionados", -1, lCuentas[Convert.ToInt32(dataGridCuentas.CurrentCell.RowIndex)].pCliente)) MessageBox.Show("se pudo eliminar la Cuenta");
