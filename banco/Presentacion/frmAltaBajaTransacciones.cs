...
namespace banco.Presentacion
{
    public partial class frmAltaBajaTransacciones : Form
    {
        ...
        private void CargarListaCuentas()
        {
            ...
            lCuentas.Clear();
            DataTable tabla = oBD.ConsultarDb("Select id_cuenta, cbu , t.nombre tipo, cl.nombre + ' ' + apellido cliente, total" +
                " from cuentas c " +
        ...
        private void CargarListaTransacciones(int id_cuenta)
        {
            ...
            DataTable tabla = oBD.ConsultarDb("Select nro_transaccion, total, fecha, activo" +
        ...
        private void MandarSql(string nombreSp, double monto)
        {
            ...
            if (oBD.EjecutarSP(nombreSp, lParametros) > 0 && oBD.EjecutarSP("sp_actualizarTotal", lParametros2)>0) MessageBox.Show("Se ha podido actualizar la transaccion");
            CargarListaTransacciones(lCuentas[lstCuentas.SelectedIndex].pId);
        }
    }
}
