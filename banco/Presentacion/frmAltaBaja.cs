...
namespace banco.Presentacion
{
    public partial class frmAltaBaja : Form
    {
        ...
        private void CargarListaCuentas()
        {
            ...
            lCuentas.Clear();
            DataTable tabla = oBD.ConsultarDb("Select cbu , t.nombre tipo, cl.nombre + ' ' + apellido cliente, activo from cuentas c" +
        ...
        private void MandarSql(string nombreSp)
        {
            ...
            if (oBD.EjecutarSP(nombreSp, lParametros) > 0) MessageBox.Show("Se ha podido actualizar la cuenta");
            CargarListaCuentas();
        }
    }
}
